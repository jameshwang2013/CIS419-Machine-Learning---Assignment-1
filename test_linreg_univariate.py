...
def plotRegLine1D( lr_model, X, y):
    ...
    plt.hold(True)
    plt.plot(X[:,1],lr_model.predict(X),'b-', label='Regression Line')
...
if __name__ == "__main__":
    ...
    theta_closed_form = np.dot(pinv(np.dot(X.T,X)),np.dot(X.T,y))  # TODO:  replace "0" with closed form solution
